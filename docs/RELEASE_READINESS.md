...
- Last verified on July 3, 2026:
  ...
  - Device archive and App Store Connect upload pass from this Mac with Apple team `FE33NM88XX`; TestFlight candidate `1.0 (9)` uploaded successfully and App Store Connect reported the package as processing. Prior build `1.0 (8)` remains the latest command-line-confirmed status check with import status `VALID` and build status `BETA_INTERNAL_TESTING`.
...
- `BUNDLE_VERSION=9 APP_STORE_CONNECT_USERNAME=rupayon244@gmail.com APP_STORE_CONNECT_KEYCHAIN_ITEM=GTA_FREE_STEM_ASC DELIVERY_ID=222e71fe-92f1-4da3-bad7-205b9eb7a3b3 bash docs/scripts/check-testflight-build-status.sh`: attempted after the build `9` upload, but stopped because Keychain password retrieval for the saved app-specific password stalled before `altool` could run. Direct `@keychain:GTA_FREE_STEM_ASC` lookup also cannot find the item by service name, so build `9` status still needs a repeat check after the Keychain item is repaired or an App Store Connect API key is available.
...
5. **App Store**
   - Verify `docs/TESTFLIGHT.md`, update App Store Connect metadata, select build `1.0 (9)` after it finishes processing, upload final screenshots, complete App Privacy and age-rating forms, and submit the app for App Review.
   ...
   - The latest IPA upload succeeded for TestFlight candidate `1.0 (9)`. Build `8` is the latest command-line-confirmed App Store Connect status (`VALID` and `BETA_INTERNAL_TESTING`); rerun the status check for build `9` before selecting it for TestFlight/App Review.
   - Processing and beta status for build `9` can be rechecked with `BUNDLE_VERSION=9 APP_STORE_CONNECT_USERNAME=rupayon244@gmail.com APP_STORE_CONNECT_KEYCHAIN_ITEM=GTA_FREE_STEM_ASC DELIVERY_ID=222e71fe-92f1-4da3-bad7-205b9eb7a3b3 bash docs/scripts/check-testflight-build-status.sh` on this Mac after Keychain/altool access is responsive.
...
- App Store Connect upload works from Xcode on this Mac. The saved app-specific password keychain item `GTA_FREE_STEM_ASC` exists, but secret retrieval stalled during the build `9` status check; repair the item or switch to an App Store Connect API key for repeatable command-line status checks. Rotate/revoke the app-specific password after release work because it was generated interactively during this setup.
